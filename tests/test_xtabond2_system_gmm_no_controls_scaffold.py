...
@pytest.mark.parity
def test_system_gmm_no_controls_scaffold_generates_native_artifacts(tmp_path: Path) -> None:
    env = os.environ.copy()
    env["PYTHONPATH"] = str(ROOT / "src")
    subprocess.run(
        [
            sys.executable,
            str(ROOT / "scripts/parity/build_xtabond2_system_gmm_no_controls_do.py"),
        ],
        cwd=tmp_path,
        env=env,
        check=True,
    ...
    subprocess.run(
        [sys.executable, str(ROOT / "scripts/parity/run_native_system_gmm_no_controls.py")],
        cwd=tmp_path,
        env=env,
        check=True,
    )

    out = tmp_path / "artifacts" / "parity" / "xtabond2" / "specs" / "system_gmm_no_controls"
