...
def main() -> None:
    ...
    plot_jobs = [
        ...
        (
            "03_prediction_path",
            lambda p: margins_prediction_plot(
                grid, x="techshare", y="pred", lower="lo", upper="hi", group="condition", save=p
            ),
        ),
        (
            "04_interaction_response",
            lambda p: interaction_plot(
                grid, x="techshare", y="pred", moderator="condition", save=p
            ),
        ),
        (
            "05_conditional_effect_path",
            lambda p: conditional_effects_plot(
                grid,
                x="techshare",
                effect="pred",
                condition="condition",
        ...
        (
            "09_panel_trajectory_map",
            lambda p: panel_spaghetti_plot(
                panel,
                entity="country",
                time="year",
                y="growth",
        ...
        (
            "12_model_health_panel",
            lambda p: model_health_panel(
                {"Hansen": 0.19, "Sargan": 0.09, "AR(1)": 0.08, "AR(2)": 0.32}, save=p
            ),
        ),
        (
            "13_scenario_response_path",
            lambda p: counterfactual_scenario_plot(
                scenario, time="year", y="pred", scenario="scenario", save=p
            ),
        ),
        (
            "14_effect_surface",
            lambda p: effect_surface_plot(surface, x="techshare", y="polity", z="pred", save=p),
        ),
        (
            "15_dynamic_persistence_path",
            lambda p: dynamic_persistence_plot(0.31, periods=20, save=p),
        ),
    ]
