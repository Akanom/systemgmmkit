...
modes = ["canonical", "unscaled", "inverse", "negative"]
rows = []

for mode in modes:
    print("\n" + "=" * 100)
    print(f"RUNNING FOD SCALE MODE: {mode}")
    print("=" * 100)

    env = os.environ.copy()
    env["PYTHONPATH"] = str(Path.cwd() / "src")
    env["SYSTEMGMMKIT_FORCE_ONESTEP"] = "1"
    env["SYSTEMGMMKIT_FOD_SCALE_MODE"] = mode
    ...
    stdout_path = OUT / f"fod_scale_{mode}_stdout.txt"
    stderr_path = OUT / f"fod_scale_{mode}_stderr.txt"
    ...
    copy_path = OUT / f"fod_scale_{mode}_coef_comparison.csv"
...
out_path = OUT / "fod_scale_mode_grid_summary.csv"
