...
stata_only = comp[comp["native_coef"].isna() & comp["stata_coef"].notna()].copy()

if compared.empty:
    raise RuntimeError("No comparable native/Stata coefficient rows found.")

expected_terms = {"L1.y", "x", "w", "_con"}
matched_term_set = set(compared["param"].astype(str))
complete_parameter_set = (
    matched_term_set == expected_terms and native_only.empty and stata_only.empty
)
...
stata_only_terms = ",".join(stata_only["param"].astype(str).tolist())
...
status = (
    ...
    if complete_parameter_set and max_abs_coef_diff <= coef_tol and max_abs_se_diff <= se_tol
    ...
    [
        {
            ...
            "stata_only_terms": stata_only_terms,
            ...
            "se_tol": se_tol,
            "message": (
                "Native System GMM matches xtabond2 on the complete expected parameter set "
                "and Windmeijer standard errors. Missing or unmatched terms fail certification."
            ),
        }
    ]
)
...
CERT_MD.write_text(
    ...
    f"`{status}`\n\n"
    "## Compared terms\n\n"
    + compared[["param", "native_coef", "stata_coef", "abs_coef_diff", "abs_se_diff"]].to_markdown(
        index=False
    )
    + "\n\n"
    "## Native-only terms\n\n"
    + (
        native_only[["param", "native_coef"]].to_markdown(index=False)
        if not native_only.empty
        else "None"
    )
    + "\n\n## Stata-only terms\n\n"
    + (
        stata_only[["param", "stata_coef"]].to_markdown(index=False)
        if not stata_only.empty
        else "None"
    )
    + "\n\n"
    "## Interpretation\n\n"
    "The maintained xtabond2 System GMM benchmark matches native systemgmmkit "
    "on the complete expected parameter set and Windmeijer-corrected standard "
    "errors within numerical tolerance. Any missing, native-only, or Stata-only "
    "parameter fails the strict comparison.\n",
    encoding="utf-8",
)
...
if not mask.any():
    strict = pd.concat(
        [strict, pd.DataFrame([{"spec": "system_gmm_baseline_controls"}])], ignore_index=True
    )
    mask = strict["spec"].astype(str).eq("system_gmm_baseline_controls")
...
strict.loc[mask, "policy_message"] = (
    "Native System GMM passed maintained xtabond2 parity for the complete expected "
    "parameter set and Windmeijer standard errors."
)
