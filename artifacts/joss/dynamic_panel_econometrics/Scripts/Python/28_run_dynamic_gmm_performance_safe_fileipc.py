...
def write_outputs(rows, diagnostics, audit):
    ...
    md += (
        ...
        "and file-based inter-process communication. Difference GMM and System GMM are benchmarked "
        "separately from static/panel/IV workflows because instrument construction, weighting matrices, "
        "and two-step covariance correction can dominate runtime. Runs that exceed the timeout are "
        "reported transparently rather than blocking artifact generation. These benchmarks are "
        "reproducibility-oriented and hardware-dependent; they are not absolute speed claims.\n"
    )
    ...
    for subset in SUBSETS:
        ...
        for benchmark, system in configs:
            for rep in range(1, REPETITIONS + 1):
                ...
                output_path = TMP / f"{benchmark.lower().replace(' ', '_')}_{size_label}_rep{rep}.json"
