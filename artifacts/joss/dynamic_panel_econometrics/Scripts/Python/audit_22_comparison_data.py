...
PATHS = {
    ...
    "stata_difference_coefficients": Path(
...
def compare_to_panel(panel: pd.DataFrame, path: Path, name: str, rows: list[dict]):
    ...
    numeric_cols = [
        c for c in common if c not in ["id", "time"] and pd.api.types.is_numeric_dtype(p[c])
    ]
    max_abs = 0.0
    worst_col = ""
    ...
    for col in numeric_cols:
        diff = (
            pd.to_numeric(p[col], errors="coerce") - pd.to_numeric(o[col], errors="coerce")
        ).abs()
    ...
    add(
        rows,
        f"{name}_matches_panel_common_columns",
        status,
        f"max_abs_diff={max_abs}",
        f"worst_col={worst_col}; common={common}",
    )
...
def main():
    ...
    add(
        rows,
        "balanced_by_id_time_count",
        "PASS" if balanced else "REVIEW",
        counts.describe().to_dict(),
        "",
    )
    ...
    add(
        rows,
        "first_period_L1_y_missing_count",
        "PASS",
        first_missing,
        "expected one missing lag per id",
    )
    add(
        rows,
        "nonfirst_period_L1_y_missing_count",
        "PASS" if nonfirst_missing == 0 else "FAIL",
        nonfirst_missing,
        "",
    )
    ...
    add(
        rows,
        "difference_term_overlap",
        "PASS"
        if sgk_diff_terms <= stata_diff_terms or sgk_diff_terms == stata_diff_terms
        else "REVIEW",
    ...
    add(
        ...
        "system_term_overlap",
        "PASS"
        if sgk_sys_terms <= stata_sys_terms or sgk_sys_terms == stata_sys_terms
        else "REVIEW",
    ...
    if comp_path.exists():
        ...
        if summary_cols:
            add(
                ...
                "INFO",
                comp[summary_cols].to_dict(orient="records"),
                "",
            )

    # Descriptive stats
    desc_cols = [
        c for c in ["y", "L1_y", "x_pred", "x_exog", "alpha_true", "eps_true"] if c in df.columns
    ]
    df[desc_cols].describe().T.to_csv(OUT / "22_panel_descriptive_statistics.csv")

    counts.reset_index(name="n_time_periods").to_csv(
        OUT / "22_panel_time_counts_by_id.csv", index=False
    )
    sdf[["id", "time", "y", "L1_y", "computed_L1_y"]].to_csv(
        OUT / "22_lag_audit_rows.csv", index=False
    )
