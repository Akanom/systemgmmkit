...
RESULTS_DIR = ROOT / "results" / "raw" / "systemgmmkit" / "dynamic_gmm_parity"
UOH_DIR = ROOT / "results" / "raw" / "systemgmmkit" / "universal_output"
ARTIFACT_DIR = ROOT / "artifacts" / "joss" / "tables"
STATA_DIR = ROOT / "replication" / "scripts" / "stata"
        ...
        try:
            ...
            if isinstance(frame, pd.DataFrame):
                ...
                if term_col and coef_col:
                    ...
                    if not s.empty:
                        s.index = s.index.map(
                            lambda x: "const" if x in ["_con", "Intercept"] else x
                        )
                        return s
        except Exception:
            pass
...
def extract_se(result: Any, params: pd.Series) -> pd.Series:
    ...
    if from_frame is not None:
        from_frame.index = from_frame.index.map(
            lambda x: "const" if x in ["_con", "Intercept"] else x
        )
...
def tidy_result(result: Any, model_name: str) -> pd.DataFrame:
    ...
    if p_from_frame is not None:
        p_from_frame.index = p_from_frame.index.map(
            lambda x: "const" if x in ["_con", "Intercept"] else x
        )
...
def extract_health_and_instruments(
    outputs: dict[str, dict[str, Any]], df: pd.DataFrame
    ...
    for model_name, bundle in outputs.items():
        ...
        try:
            ...
            instrument_health = result.check_instrument_health()
            hdict.update(
                {
                    "instrument_group_ratio": instrument_health.ratio,
                    "instrument_health_status": instrument_health.status,
                    "instrument_health_warning": instrument_health.warning,
                    "instrument_health_recommendation": instrument_health.recommendation,
                }
            )
        except Exception as exc:
            hdict = {"error": str(exc)}
...
def write_stata_references(diff_spec: Any, sys_spec: Any) -> None:
    ...
    manual_source = STATA_DIR / "jss_dynamic_gmm_manual.do"
    if not manual_source.exists():
        raise FileNotFoundError(f"Missing portable Stata reference: {manual_source}")
    do_path = STATA_DIR / "22_stata_xtabond2_parity_reference.do"
    do_path.write_text(manual_source.read_text(encoding="utf-8"), encoding="utf-8")
...
def write_uoh_table(result_tables: dict[str, pd.DataFrame]) -> None:
    ...
    for model_name, table in result_tables.items():
        ...
        model = from_coefficient_table(
            ...
            depvar=Y,
            statistics={
                "source": "systemgmmkit 1.0.0 controlled Dynamic GMM benchmark",
            },
            diagnostics={
                "dataset": "Controlled synthetic dynamic panel",
                "entity": ENTITY,
...
def main() -> None:
    if getattr(sgk, "__version__", "unknown") != "1.0.0":
        raise RuntimeError("Controlled GMM replication requires systemgmmkit 1.0.0.")
    ...
    status = {
        ...
        "systemgmmkit_installation": "installed distribution",
        "status": "PASS",
        "models": list(result_tables.keys()),
        "dataset": CONTROLLED_DATA.relative_to(ROOT).as_posix(),
        "outputs": {
